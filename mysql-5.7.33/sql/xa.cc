    ...
    while ((got= hton->recover(hton, info->list, info->len)) > 0)
    {
      ...
      for (int i= 0; i < got; i++)
      {
        ...
        if (!x) // not "mine" - that is generated by external TM
        {
          ...
          XID *xid= info->list + i;
          sql_print_information("ºöÂÔ xid %s", xid->xid_to_str(buf));
#endif
            ...
            tc_heuristic_recover == TC_HEURISTIC_RECOVER_COMMIT)
        {
          ...
          char buf[XIDDATASIZE * 4 + 6]; // see xid_to_str
          XID *xid= info->list + i;
          sql_print_information("Ìá½» xid %s", xid->xid_to_str(buf));
#endif
        ...
        else
        {
          ...
          char buf[XIDDATASIZE * 4 + 6]; // see xid_to_str
          XID *xid= info->list + i;
          sql_print_information("»Ø¹ö xid %s", xid->xid_to_str(buf));
#endif
...
int ha_recover(HASH *commit_list)
{
  ...
  if (info.commit_list)
    sql_print_information("[¿ªÊ¼±ÀÀ£»Ö¸´] crash recovery...");
  ...
  if (info.commit_list)
    sql_print_information("[±ÀÀ£»Ö¸´]Crash recovery Íê³É.");
  DBUG_RETURN(0);
}
